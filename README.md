...
5. 确认“LeechBlock NG — 全局理由门（本地版）”已启用，版本为 **1.7.3.6**，允许在目标网站上运行。需要无痕支持时另行开启“允许在无痕模式下运行”。
...
51 项回归测试覆盖实际后台和页面脚本，包括内置页提交、5 秒边界、输入法、来源校验、共享期限、旧错误页迁移和显示设置。Chrome API 使用测试替身，仍需按上述步骤进行真实浏览器验收。CI 另行运行旧服务兼容检查。
