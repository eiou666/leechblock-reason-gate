...
2. 进入页面后等待 **5 秒**，不显示数字倒计时；这期间点击按钮无反应，Enter 也不能提前提交。
3. 5 秒后按钮可点击。填写至少 **5 个非空白字符**，点击“确定”或按 Enter，直接进入原网址（Ctrl+Enter 仍兼容；Shift+Enter 换行；输入法选字回车不提交）。没有单独的倒计时界面，也不会因为等够时间自动放行。
