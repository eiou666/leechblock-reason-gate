...
gReasonSessions.ready.catch(error => warn("Cannot restore shared grants: " + error));
browser.storage.onChanged.addListener((changes, area) => gReasonSessions.changed(changes, area));
...
function initTab(id) {
	if (gTabs[id]) {
		...
		gTabs[id] = {
			...
			referrer: "",
			url: "about:blank",
			incog: !!browser.extension?.inIncognitoContext,
			audible: false,
			focused: false,
			loaded: false,
			loadedTime: 0
...
function retrieveOptions(update) {
	...
	gOptionsReady = browser.storage.local.get("sync").then(onGotSync).catch(onError);
	return gOptionsReady;
	...
	function onGotSync(options) {
		...
		return Promise.all([gStorage.get(), gReasonSessions.ready]).then(([options]) => onGot(options));
	}
...
function checkTab(id, isBeforeNav, isRepeat) {
	...
	for (let set = 1; set <= gNumSets; set++) {
		...
		if (allowHost && allowPath && allowedSet == set && !reasonSharedPolicy(set, gOptions)) {
		...
		if (testURL(pageURL, referrer, blockRE, allowRE, referRE, allowRefers)
			...
			// Check a set-wide deadline after matching the URL. No opener dependency.
			const grant = gReasonSessions.get(set, incog, reasonSharedPolicy(set, gOptions), now);
			if (grant && doBlock && !lockdown && !withinMinBlock && !isInternalPage) {
				const secsLeft = grant.expiresAt - now;
				if (secsLeft < gTabs[id].secsLeft) {
					gTabs[id].secsLeft = secsLeft;
					gTabs[id].secsLeftSet = set;
					gTabs[id].showTimer = showTimer;
				}
				continue; // Other block sets must still be checked.
			}
...
function discardRemainingTime() {
	...
	if (set) {
		if (reasonSharedPolicy(set, gOptions)) {
			gReasonSessions.revoke(set, gTabs[gActiveTabId].incog).catch(error => warn(error));
			return;
		}
...
async function allowBlockedPage(id, url, set, autoLoad, sourceURL) {
	...
	if (policy) {
		...
		try {
			await gReasonSessions.issue(set, gTabs[id].incog, policy, now);
			// If the schedule ended during countdown, navigation applies the new set.
			if (autoLoad) await browser.tabs.update(id, { url });
			await resumeSharedGateTabs();
		} catch (error) { warn("Cannot save shared grant: " + error); }
		return;
	}
...
// Also release already-open gate pages; never issue/renew grants here.
...
function handleMessage(message, sender, sendResponse) {
	...
	if (["blocked", "delayed", "password"].includes(message.type) && sender.tab) {
		// A content-script message can wake a suspended worker before options load.
		if (!gGotOptions) {
			(gOptionsReady || retrieveOptions()).then(() => {
				if (gGotOptions) handleMessage(message, sender, sendResponse);
				else sendResponse({});
			});
			return true;
		}
		initTab(sender.tab.id);
		gTabs[sender.tab.id].incog = !!sender.tab.incognito;
	}
	...
	switch (message.type) {
		...
		case "blocked":
			...
			resumeSharedGate({ ...sender.tab, url: sender.url }).catch(error => warn(error));
		...
		case "delayed":
			...
			allowBlockedPage(sender.tab.id,
					message.blockedURL,
					message.blockedSet,
					gOptions[`delayAutoLoad${message.blockedSet}`], sender.url).catch(error => warn(error));
			break;
